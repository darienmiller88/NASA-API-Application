...
func renderCurrentAPODPicture(c *gin.Context){
	...
	var apodURL string  = webData["url"].(string)
	var isAPicture bool = strings.Contains(apodURL, "jpg")//Check to see if the url is a jpg.

	renderHTML(c, response.StatusCode, webData, isAPicture)
}
